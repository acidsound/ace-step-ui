...
import { X, Play, Pause, Download, Wand2, Image as ImageIcon, Music, Video, Loader2, Palette, Layers, Zap, Type, Monitor, Aperture, Activity, Circle, Grid, Box, BarChart2, Waves, Disc, Upload, Plus, Trash2, Settings2, MousePointer2, Search, ExternalLink, Sun, Film, Minus, ArrowLeft } from 'lucide-react';
...
export const VideoGeneratorModal: React.FC<VideoGeneratorModalProps> = ({ isOpen, onClose, song }) => {
  ...
  const [noraebangConfig, setNoraebangConfig] = useState<NoraebangConfig>({
    ...
    yPos: 85,
    primaryColor: '#ffffff',
    glowColor: '#ec4899' // matches default primary pink
  });
  ...
  const loadFFmpeg = useCallback(async () => {
    ...
    try {
      ...
      ffmpeg.on('progress', ({ progress }) => {
        if (exportStageRef.current === 'encoding') {
          // Scale encoding progress from 75% to 95%
          setExportProgress(75 + (progress * 20));
        }
      });
  ...
  const renderOffline = async () => {
    ...
    for (let frameIndex = 0; frameIndex < totalFrames; frameIndex++) {
      ...
      currentTexts.forEach(layer => {
        ...
        // Stroke layer for better visibility
        ctx.strokeStyle = 'black';
        ctx.lineWidth = 4;
        ctx.strokeText(layer.text, xPos, yPos);
      ...
      // Capture frame using blob for better performance
      await new Promise<void>((resolve, reject) => {
        canvas.toBlob(async (blob) => {
          ...
          try {
            ...
            await ffmpeg.writeFile(`frame${String(frameIndex).padStart(6, '0')}.jpg`, new Uint8Array(arrayBuffer));
            resolve();
          } catch (err) {
            reject(err);
          }
      ...
      if (frameIndex % 5 === 0 || frameIndex === totalFrames - 1) {
        setExportProgress(15 + (frameIndex / totalFrames) * 55);
      }
    }
    ...
    // Write audio file - use .wav extension as it is a WAV file
    try {
      await ffmpeg.writeFile('audio.wav', new Uint8Array(audioDataCopy));
    } catch (err) {
      throw new Error(`Failed to write audio file: ${err instanceof Error ? err.message : String(err)}`);
    }
  ...
  const renderLoop = () => {
    ...
    // Fix: Use audio time for preview sync, fallback to wall clock only if no audio
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/90 backdrop-blur-md p-0 md:p-4 animate-in fade-in duration-200">
      ...
      <div className={`bg-suno-card w-full h-full md:max-w-7xl md:h-[90vh] md:rounded-2xl border-0 md:border border-white/10 overflow-hidden shadow-2xl relative ${isMobile ? 'flex flex-col' : 'flex'}`}>
        ...
        <div className={`${isMobile ? 'flex-1 overflow-hidden' : 'w-96'} bg-suno-panel ${isMobile ? '' : 'border-r border-white/5'} flex flex-col z-20`}>
          ...
          <div className="flex border-b border-white/5">
            {[
              ...
              { id: 'style', label: 'Styles', icon: <Palette size={14} /> },
          ...
          <div className="flex-1 overflow-y-auto custom-scrollbar p-4 md:p-6 space-y-4 md:space-y-6">
            ...
            {activeTab === 'style' && (
              <div className="space-y-6">
                ...
                {/* Pexels API Key Section - Only show when prompted */}
                {showPexelsApiKeyInput && (
                  <div className="space-y-3 bg-zinc-900/50 p-4 rounded-xl border border-white/5 animate-in fade-in slide-in-from-top-2">
                    <div className="flex items-center justify-between">
                      <label className="text-xs font-bold text-zinc-500 uppercase">Pexels API Key</label>
                      <button onClick={() => setShowPexelsApiKeyInput(false)} className="text-zinc-500 hover:text-white">
                        <X size={14} />
                      </button>
                    </div>
                    <div className="flex gap-2">
                      <input
                        ...
                        className="flex-1 bg-zinc-950 rounded-lg px-4 py-2 text-sm text-white border border-white/10 placeholder-zinc-500 focus:outline-none focus:border-pink-500/50"
                      />
                      <button
                        onClick={() => {
                    ...
                    <div className="flex items-center justify-between">
                      <p className="text-[10px] text-zinc-500">Stored locally in your browser.</p>
                      <a
          ...
          {/* Sidebar Footer with Export Button - Restored from main */}
          <div className="p-6 border-t border-white/5 space-y-4">
            {ffmpegLoading ? (
              <div className="w-full h-12 bg-zinc-800/50 rounded-xl flex items-center justify-center border border-white/5">
                ...
                <div className="flex items-center gap-2 z-10 text-white font-bold text-sm">
                  {exportStage === 'capturing' ? (
                    <>
                      ...
                      Rendering frames {exportProgress.toFixed(1)}%
                    </>
                  ) : (
                    <>
                      <Loader2 className="animate-spin text-blue-400" size={16} />
                      {exportProgress < 95 ? 'Encoding (be patient)...' : `Encoding MP4 ${exportProgress.toFixed(1)}%`}
                    </>
                  )}
                </div>
              </div>
            ...
            <p className="text-[10px] text-zinc-600 text-center">
              {ffmpegRef.current ? 'Encoder ready • ' : ''}Offline rendering - faster than real-time.
            </p>
          </div>
        </div>

        {/* PEXELS BROWSER OVERLAY - Restore to fixed modal like main */}
        {showPexelsBrowser && (
          <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 animate-in fade-in duration-200">
            <div className="bg-zinc-900 w-full max-w-4xl max-h-[85vh] rounded-2xl border border-white/10 flex flex-col overflow-hidden shadow-2xl">
              {/* Pexels Header */}
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="text-white font-bold">
                      {pexelsTarget === 'albumArt' ? 'Select Center Image' : (pexelsTab === 'photos' ? 'Select Background Photo' : 'Select Background Video')}
                    </h3>
                    <p className="text-zinc-500 text-[10px]">Free high-quality stock media from Pexels</p>
                  </div>
                </div>
                <button onClick={() => setShowPexelsBrowser(false)} className="p-2 hover:bg-white/10 rounded-full transition-colors text-zinc-400 hover:text-white">
                  <X size={20} />
                </button>
              </div>
              ...
              <div className="p-4 bg-zinc-900/30 space-y-3">
                <div className="flex gap-2">
                  {pexelsTarget !== 'albumArt' && (
                    <div className="flex bg-zinc-800 p-1 rounded-lg">
                      <button
                        ...
                        className={`px-3 py-1.5 rounded-md text-xs font-bold transition-all ${pexelsTab === 'photos' ? 'bg-emerald-600 text-white shadow-lg' : 'text-zinc-500 hover:text-zinc-300'}`}
                      >
                        Photos
                      </button>
                      <button
                        onClick={() => { setPexelsTab('videos'); searchPexels(pexelsQuery, 'videos'); }}
                        className={`px-3 py-1.5 rounded-md text-xs font-bold transition-all ${pexelsTab === 'videos' ? 'bg-emerald-600 text-white shadow-lg' : 'text-zinc-500 hover:text-zinc-300'}`}
                      >
                        Videos
                      </button>
                    </div>
                  ...
                  <div className="flex-1 flex gap-2">
                    <input
                      ...
                      placeholder="Search abstract, nature, neon..."
                      className="flex-1 bg-zinc-800 rounded-lg px-4 py-2 text-sm text-white border border-white/10 placeholder-zinc-500 focus:outline-none focus:border-emerald-500/50"
                    />
                    <button
                      onClick={() => searchPexels(pexelsQuery, pexelsTab)}
                      disabled={pexelsLoading}
                      className="px-6 py-2 bg-emerald-600 hover:bg-emerald-700 rounded-lg text-white font-bold text-sm flex items-center gap-2 transition-colors disabled:opacity-50"
                    >
                ...
                <div className="flex flex-wrap gap-2">
                  {['abstract', 'nature', 'cyberpunk', 'liquid', 'space', 'neon', 'minimal'].map(tag => (
                    <button
                      key={tag}
                      onClick={() => { setPexelsQuery(tag); searchPexels(tag, pexelsTab); }}
                      className="px-3 py-1 bg-zinc-800 hover:bg-zinc-700 rounded-full text-[10px] text-zinc-400 hover:text-white capitalize transition-colors"
                    >
                      {tag}
                    </button>
                  ))}
              ...
              <div className="flex-1 overflow-y-auto p-4 custom-scrollbar bg-black/20">
                {pexelsLoading ? (
                  <div className="flex flex-col items-center justify-center h-64 gap-4">
                    ...
                    <p className="text-zinc-500 text-sm animate-pulse">Searching Pexels...</p>
                  </div>
                ) : pexelsTab === 'photos' ? (
                  <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                    {pexelsPhotos.map(photo => (
                      <button
                        ...
                        className="relative group rounded-xl overflow-hidden aspect-video bg-zinc-800 ring-1 ring-white/5 hover:ring-emerald-500/50 transition-all shadow-lg"
                      >
                        <img src={photo.src.large} alt="" className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-110" />
                        <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                          <div className="bg-emerald-600 text-white text-[10px] font-bold px-4 py-1.5 rounded-full transform translate-y-4 group-hover:translate-y-0 transition-transform">Select Photo</div>
                        </div>
                        <div className="absolute bottom-0 left-0 right-0 p-2 bg-gradient-to-t from-black to-transparent opacity-0 group-hover:opacity-100 transition-opacity">
                          <p className="text-[10px] text-zinc-300 truncate">© {photo.photographer}</p>
                        </div>
                      </button>
                    ))}
                  </div>
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                    {pexelsVideos.map(video => (
                      <button
                        ...
                        className="relative group rounded-xl overflow-hidden aspect-video bg-zinc-800 ring-1 ring-white/5 hover:ring-emerald-500/50 transition-all shadow-lg"
                      >
                        <img src={video.image} alt="" className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-110" />
                        <div className="absolute top-2 right-2 bg-black/60 px-2 py-0.5 rounded text-[10px] text-white font-bold flex items-center gap-1">
                          <Video size={10} /> VIDEO
                        </div>
                        <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                          <div className="bg-emerald-600 text-white text-[10px] font-bold px-4 py-1.5 rounded-full transform translate-y-4 group-hover:translate-y-0 transition-transform">Select Video</div>
                        </div>
                        <div className="absolute bottom-0 left-0 right-0 p-2 bg-gradient-to-t from-black to-transparent opacity-0 group-hover:opacity-100 transition-opacity">
                ...
                {!pexelsLoading && pexelsPhotos.length === 0 && pexelsTab === 'photos' && (
                  <div className="text-center py-20">
                    <p className="text-zinc-500 text-sm">No photos found for "{pexelsQuery}"</p>
                  </div>
                )}
                {!pexelsLoading && pexelsVideos.length === 0 && pexelsTab === 'videos' && (
                  <div className="text-center py-20">
                    <p className="text-zinc-500 text-sm">No videos found for "{pexelsQuery}"</p>
                  </div>
                )}
              </div>

              {/* Footer */}
              <div className="px-6 py-3 border-t border-white/5 bg-zinc-900 flex justify-between items-center">
                <p className="text-[10px] text-zinc-600">
                  Search results powered by <span className="text-emerald-500 font-bold">Pexels</span>
                </p>
                <button
        ...
        {!isMobile && (
          <div className="flex-1 bg-black relative flex flex-col items-center justify-center p-8 bg-dot-pattern">
            {/* Canvas Container */}
            <div className={`relative aspect-video w-full max-w-4xl shadow-2xl rounded-xl overflow-hidden ring-1 ring-white/10 ${isExporting ? 'ring-pink-500 ring-2' : ''}`}>
              <canvas
                ref={canvasRef}
                width={1920}
                height={1080}
                  ...
                  <div className="text-center space-y-4">
                    ...
                    <div>
                      <h3 className="text-xl font-bold text-white mb-1">
                        {exportStage === 'capturing' ? 'Capturing Frames' : exportStage === 'encoding' ? 'Encoding Video' : 'Finalizing'}
                      </h3>
                      <p className="text-zinc-400 text-sm">Please keep this window open.</p>
                    </div>
                  </div>
                </div>
              )}
            ...
            <div className="mt-8 flex items-center gap-6">
              <button
                onClick={togglePlay}
                disabled={isExporting}
                className="w-16 h-16 rounded-full bg-white hover:bg-zinc-200 text-black flex items-center justify-center transition-transform hover:scale-105 shadow-xl disabled:opacity-50 disabled:hover:scale-100"
              >
              ...
              <div className="flex flex-col gap-1">
                ...
                <div className="text-zinc-500 text-xs font-mono">1920 x 1080 • 30fps</div>
              </div>
